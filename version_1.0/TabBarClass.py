## {{{ http://code.activestate.com/recipes/577261/ (r4)
###################################################
...
# a base tab class
class Tab(Frame):
    def __init__(self, master, name, color=None):
...
class TabBar(Frame):
    ...
    def show(self):
        self.grid(row=0,column=0, sticky=E+W,padx=3,pady=3)
    ...
    def add(self, tab):
        ...
        if tab.color!=None:
            b.config(highlightbackground=tab.color)
        b.grid(row=0,column=self.mycolumns,padx=3,pady=3) 
        self.mycolumns+=1                                               # pack the buttont to the left mose of self
    ...
    def delete(self, tabname):
        ...
        if tabname == self.current_tab:
            self.current_tab = None
            self.tabs[tabname].grid_forget()
            del self.tabs[tabname]
            self.switch_tab(self.tabs.keys()[0])
        
        else: del self.tabs[tabname]
        
        self.buttons[tabname].grid_forget()
...
if __name__ == '__main__':
    ...
    root.mainloop()
## end of http://code.activestate.com/recipes/577261/ }}}
